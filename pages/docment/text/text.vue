<template>
	<cu-sys :loading="loading" doc title="文本">
		<view class="cu-container">
			...
			<!-- 尺寸 -->
			<cu-title title="尺寸" depth="2" isIcon toc></cu-title>
			<view class="paragraph">
				添加包含尺寸的Class来控制文字大小。
				<cu-code>.text-{size}</cu-code>
			</view>
			<view class="paragraph">
				你可以在
				<cu-code>@/colorui/scss/_var.scss</cu-code>
				中配置
				<cu-code>$fontsize</cu-code>
				来改变对应的文字大小。
			</view>
			<view class="blockquote text-sm">
				跨大屏端建议使用px为单位,移动端建议使用rpx为单位,关于怎样配置项目的单位，请参考
				<navigator class="text-main-a px-2" url="/pages/docment/navbar/demo?id=2">
					rpx与px的转换
						...
						<view class="p-3 mt-1 flex-bar" v-for="(item, index) in size" :key="index">
							...
							<view class="flex-sub text-center px-4">
								<view class="p-3" :class="item.class">
									<text v-if="item.tag" class="cu-tag bg-blue round">{{ item.tag }}</text>
					...
					<view class="cu-table table-border table-full box-bg">
						<view class="cu-table-header">
							<view class="cu-table-tr box-bg-sub">
								<view class="cu-table-th" style="width: 200rpx;">类名</view>
								<view class="cu-table-th" style="width: 100rpx;">大小(px)</view>
								<view class="cu-table-th">说明</view>
							</view>
						</view>
						<view class="cu-table-body">
							<view class="cu-table-tr" v-for="(item, index) in size" :key="index">
								<view class="cu-table-td">
									<view>.{{ item.class }},</view>
									<view>.text-{{ item.size }}</view>
								</view>
								<view class="cu-table-td">{{ item.size }}</view>
								<view class="cu-table-td">{{ item.info }}</view>
							</view>
						</view>
					</view>
				</view>
			...
			<!-- 颜色 -->
			<cu-title title="颜色" depth="2" isIcon toc></cu-title>
			<view class="paragraph">
				添加包含颜色的Class来控制文字颜色。
				<a href="#简介">简介</a>
				<cu-code>.text-{color}</cu-code>
			</view>
			<view class="cu-table table-border table-full box-bg">
				<view class="cu-table-header">
					<view class="cu-table-tr box-bg-sub">
				...
				<view class="cu-table-body">
					<view class="cu-table-tr" v-for="(item, index) in color" :key="index">
						<view class="cu-table-td">
					...
					<view class="cu-table-tr" v-for="(item, index) in themeColor" :key="index+100">
						<view class="cu-table-td">
					...
					<view class="cu-table-tr" v-for="(item, index) in themetext" :key="index+200">
						<view class="cu-table-td">
							<view>.text-{{ item.name }}</view>
						</view>
						<view class="cu-table-td">
					...
					<view class="cu-table-tr bg-dark-3" v-for="(item, index) in gray" :key="index+300">
						<view class="cu-table-td">
							<view>.text-{{ item.name }}</view>
						</view>
						<view class="cu-table-td">
					...
					<view class="cu-table-tr bg-white" v-for="(item, index) in dark" :key="index+400">
						<view class="cu-table-td">
							<view>.text-{{ item.name }}</view>
						</view>
						<view class="cu-table-td">
			...
			<!-- 截断 -->
			<cu-title title="截断" depth="2" isIcon toc></cu-title>
			<view class="paragraph">
				添加
				<cu-code>.text-cut</cu-code>
				来控制段落的截断。
			</view>
			<block v-if="demo">
				<view class="box-bg radius p-3 my-4" style="width: 300px;">
					<view class="text-cut">{{ longText }}</view>
				</view>
			</block>
			<cu-code tag="pre" lang="html" :content="code1" v-if="!demo||pc"></cu-code>
			<view class="paragraph">
				多行截断使用
				<cu-code>.text-linecut-{line}</cu-code>
				支持2~10行的截断，想实现更多行的截断可以试试在行内样式里写
				<cu-code>-webkit-line-clamp: 11;</cu-code>
			</view>
			<block v-if="demo">
				<view class="box-bg radius p-3 my-4" style="width: 300px;">
			...
			<cu-code tag="pre" lang="html" :content="code2" v-if="!demo||pc"></cu-code>

			<!-- 对齐 -->
			<cu-title title="对齐" depth="2" isIcon toc></cu-title>
			<view class="paragraph">
				添加
				<cu-code>.text-{ align }</cu-code>
				来控制文本的对齐。
			</view>
			<block v-if="demo">
				<view class="box-bg radius p-3 my-4">
			...
			<cu-code tag="pre" lang="html" :content="code3" v-if="!demo||pc"></cu-code>

			<!-- 辅助类 -->
			<cu-title title="辅助类" depth="2" isIcon toc></cu-title>
			<view class="p-3 my-3  box-bg radius text-center" v-for="(item, index) in util" :key="index">
				<view class="p-3">
				...
				<view class="text-grey border-top p-3 text-sm">{{ item.info }} <cu-code>.{{item.class}}</cu-code></view>
			</view>
		</view>
