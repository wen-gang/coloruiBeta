<template>
	<view class="doc-card">
		<navigator v-if="data.icon" :url="data.url" class="flex radius" :class="['bg-' + colorname + '-gradient']">
		...
		<navigator v-else :url="data.url" class="radius block border" :class="['bg-' + colorname + '-light','border-' + colorname]">
			<view class="">
				<view class="text-lg text-light py-5 text-center">{{ data.title }}</view>
			</view>
		</navigator>
...
export default {
	...
	created() {
		...
		this.colorname = colorlist[Math.floor(Math.random() * colorlist.length)].name;
	}
};
</script>
