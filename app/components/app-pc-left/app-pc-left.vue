<template>
	<view class="nav-box">
		<block v-for="(nav, navindex) in navlist" :key="navindex">
			<view class="flex-bar py-2 pl-4"><ui-title :title="nav.title" isIcon depth="2"></ui-title></view>
			<view class="nav-ul" v-if="nav.subnav">
				<view
					v-for="(item, index) in nav.subnav"
					:key="index"
					@click="_to(`/pages/document/${item.name}`)"
					class="nav-li"
				...
				>
					...
					<view class="ui-btn bg-none icon sm"><text class="uicon-angle"></text></view>
				</view>
			</view>
		</block>
	</view>
