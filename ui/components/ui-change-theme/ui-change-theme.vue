<template>
	<view class="">
		<view class="ui-menu sm-border" :class="[{ radius: radius },{ border: border }, { 'radius-sm': radius == 'sm' }, { 'radius-lg': radius == 'lg' }]">
			<!-- #ifndef APP-PLUS -->
			<view class="ui-menu-item" @tap="setTheme(sys_theme == 'auto' ? 'light' : 'auto')">
				<view class="ui-menu-item-content">跟随系统</view>
				<view class="action"><ui-switch :checked="sys_theme == 'auto'"></ui-switch></view>
			</view>
			<!-- #endif -->
			...
			<view class="ui-menu-item" v-if="sys_theme != 'auto'">
				<view class="flex flex-center p-5 text-center flex-sub">
					<view class="px-5">
						<view class="mobile-phone border" @tap="setTheme('light')" :class="{ cur: sys_theme == 'light' }">
					...
					<view class="px-5">
						<view class="mobile-phone border dark" @tap="setTheme('dark')" :class="{ cur: sys_theme == 'dark' }">
							<view class="ui-BG-Main radius"></view>
			...
			<view class="ui-menu-item">
				...
				<view class="action" @tap="$Modal.show('ColorPicker')"><ui-radio :bg="`bg-${sys_main} shadow-${sys_main} shadow-sm`"></ui-radio></view>
			</view>
		</view>
		
		<ui-modal name="ColorPicker">
			<view class="ui-grid ui-cols-6 p-3">
				<view class="text-center ui-item py-2 my-2" v-for="(item, index) in color" :key="index" @tap="chooseColor(item)">
					<ui-radio :value="sys_main" :label="item" :bg="`bg-${item} shadow-${item} shadow-sm`" :unbg="`bg-${item} shadow-${item} shadow-sm`"></ui-radio>
				</view>
			</view>
		</ui-modal>
	</view>
